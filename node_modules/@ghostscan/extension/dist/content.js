...
function detectSaaSApplication() {
    try {
        ...
        if (hasLoginForm || hasSignupForm || hasDashboard || hasUserMenu) {
            ...
            const thirdPartyCookies = document.cookie.split(';').some(cookie => !cookie.includes(domain) && cookie.includes('='));
            ...
            const oauthProviders = ['google', 'facebook', 'github', 'twitter', 'linkedin'];
            const hasOAuth = oauthProviders.some(provider => document.querySelector(`[href*="${provider}"], [src*="${provider}"]`));
            if (hasOAuth) {
...
async function handleDashboardCommunication(event) {
    try {
