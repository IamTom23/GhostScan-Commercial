...
class GhostScanBackground {
        ...
        try {
            ...
            history.forEach(item => {
                if (item.url) {
                    ...
                    const isSaaS = saasPatterns.some(pattern => domain.includes(pattern) || item.title?.toLowerCase().includes(pattern));
                    ...
                    if (isSaaS && !shouldSkip) {
                        ...
                        if (existingIndex === -1) {
                            ...
                            const dataTypes = ['saas_application'];
    ...
    async generateComprehensiveReport(cookies, oauthData, historyData, trackingData) {
        ...
        detectedApplications.forEach((detectedApp) => {
            const app = {
                id: detectedApp.id,
        ...
        if (historyData.saasApplications) {
            historyData.saasApplications.forEach((saasApp) => {
    ...
    async storeDetectedApplication(message) {
        try {
            ...
            const existingIndex = detectedApplications.findIndex((app) => app.domain === domain);
            const newApp = {
                id: `saas_${domain}`,
    ...
    async storeOAuthDetection(message) {
        try {
    ...
    async storeTrackingDetection(message) {
        try {
    ...
    async storeFormDetection(message) {
        try {
